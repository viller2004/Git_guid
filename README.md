...
<details>
<summary>Делаем pull request</summary>
...
💡 Перед созданием нового пул-реквеста считается хорошей практикой перейти в главную ветку, «подтянуть» `git pull` в неё изменения, а затем добавить эти изменения в вашу ветку с помощью `git merge main`.
...
`git branch -a` Показывает все известные ветки, как локальные, так и удалённые (в origin, или на GitHub).

`git branch ИМЯ` Создает новую ветку с наванием ИМЯ.

`git branch -d ИМЯ` Удаляет ветку с названием ИМЯ, но только если она является частью `main`.

`git branch -D ИМЯ` Удаляет ветку с названием ИМЯ, даже если она не объединена с `main`.
...
`git merge <имя ветки>` объедини ветку `<имя ветки>` с **`текущей активной веткой.`**
